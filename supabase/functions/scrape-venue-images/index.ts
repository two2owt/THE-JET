...
Deno.serve(async (req) => {
  ...
  try {
    // Require an authenticated admin caller — this function uses the service
    // role to mutate deals and consumes the paid Firecrawl quota.
    ...
    try {
      ...
      const privateIPRegex =
        /^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|127\.|169\.254\.|localhost$|::1$|fe80:)/i;
      ...
      if (ipRegex.test(hostname)) {
        throw new Error(
          "Direct IP addresses are not allowed. Please use a domain name.",
        );
      }
    } catch (urlError) {
