...
Deno.serve(async (req) => {
  ...
  try {
    // Authenticate the caller and require admin role — analytics endpoints
    // must never be readable by anon or non-admin users.
    ...
    if (roleError || !isAdmin) {
      return new Response(
        ...
        { status: 403, headers: { ...rateLimitHeaders, "Content-Type": "application/json" } }
      );
    }
