...
const FinishOnboardingEmail = ({ name, attempt = 1 }: FinishOnboardingProps) => (
    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={{ margin: '8px 0 28px' }}>
          <Text style={listItem}>🍸 Pick the food, drinks, nightlife and events you actually care about</Text>
          <Text style={listItem}>📍 Get alerts when deals go live near you</Text>
          <Text style={listItem}>👯 Show up on your crew's map and meet them out</Text>
        </Section>
        <Button style={button} href={`${SITE_URL}/onboarding`}>
          Finish my profile
        </Button>
        <Text style={footer}>
          Already done? Ignore this — you're all set. Questions? Just reply and a
          real human will get back to you.
        </Text>
      </Container>
    </Body>
  </Html>
...
const text = { fontSize: '16px', lineHeight: '26px', color: '#333333', margin: '0 0 16px' }
const listItem = { fontSize: '15px', lineHeight: '24px', color: '#333333', margin: '0 0 8px' }
const button = {
...
const footer = { fontSize: '13px', lineHeight: '20px', color: '#777777', margin: '28px 0 0' }
