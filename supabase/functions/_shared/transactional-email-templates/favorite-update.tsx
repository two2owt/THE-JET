...
const FavoriteUpdateEmail = ({
  ...
  const url =
    ctaUrl && ctaUrl.startsWith("http")
      ? ctaUrl
      : `${SITE_URL}${ctaUrl ?? "/favorites"}`;
  ...
  if (!bodyText && eventType === "ending_soon" && expiresAt) {
    ...
    if (mins > 0 && mins < 180)
      contextLine = `${dealTitle} ends in ${mins} minutes.`;
  }
    ...
    <Html lang="en" dir="ltr">
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Text style={text}>
            {name ? `Hi ${name}, ` : ""}a spot on your favorites list just
            posted an update.
          </Text>
          <Section style={card}>
          ...
          <Text style={footer}>
            You're receiving this because you favorited this venue or deal in{" "}
            {SITE_NAME}. Manage email preferences in Settings → Notifications.
          </Text>
        </Container>
      </Body>
    </Html>
