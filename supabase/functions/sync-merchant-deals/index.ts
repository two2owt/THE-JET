...
const MerchantDealSchema = z.object({
  ...
  onboarding_started_at: z
    .string()
    .datetime({ offset: true })
    .optional()
    .nullable(),
  onboarding_completed_at: z
    .string()
    .datetime({ offset: true })
    .optional()
    .nullable(),
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    return new Response(
      JSON.stringify({
        ...
        message:
          "sync-merchant-deals is reachable. POST a webhook payload with x-webhook-secret or Authorization: Bearer <secret>.",
      }),
  ...
  try {
    ...
    if (!providedSecret) {
      console.error(
        `[${reqId}] No secret provided. Headers seen: x-webhook-secret=${!!headerSecret}, authorization=${!!authHeader}`,
      );
      return new Response(
        JSON.stringify({
          error: "Unauthorized",
    ...
    if (providedSecret !== expectedSecret) {
      console.error(
        `[${reqId}] Secret mismatch. providedLen=${providedSecret.length} expectedLen=${expectedSecret.length}`,
      );
      return new Response(
        JSON.stringify({
          error: "Unauthorized",
    ...
    console.log(
      `[${reqId}] payload keys: ${Object.keys(rawPayload ?? {}).join(",")}`,
    );
    ...
    console.log(
      `[${reqId}] Processing ${payload.action} for deal ${payload.deal.id} (active=${payload.deal.active})`,
    );
    ...
    switch (action) {
      case "create": {
        ...
        console.log(
          `[${reqId}] Mapped deal_type "${deal.deal_type}" -> "${mappedDealType}"`,
        );
      ...
      case "update": {
        ...
        const mappedDealType = mapDealType(deal.deal_type);
        console.log(
          `Mapped deal_type "${deal.deal_type}" to "${mappedDealType}"`,
        );
