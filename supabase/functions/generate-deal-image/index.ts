...
const dealImageSchema = z.object({
  ...
  dealType: z
    .string()
    .max(50, "Deal type must be less than 50 characters")
    .optional(),
});
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!hasAdminRole) {
      throw new Error(
        "Unauthorized: Admin role required to generate deal images",
      );
    }
    ...
    if (
      ...
      description.toLowerCase().includes("tasting") ||
      description.toLowerCase().includes("oyster")
    ) {
      ...
      description.toLowerCase().includes("cocktail")
    ) {
