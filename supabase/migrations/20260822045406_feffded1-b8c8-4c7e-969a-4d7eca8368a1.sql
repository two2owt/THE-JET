...
BEGIN
  ...
  FOR th IN SELECT * FROM public.map_sync_latency_thresholds WHERE enabled LOOP
    ...
    -- Not enough traffic to judge the stage. An alert that is still open with no
    -- supporting samples is stale evidence, so let it auto-resolve instead of
    -- pinning the dashboard to "degraded" forever.
    IF m.samples IS NULL OR m.samples < th.min_samples THEN
