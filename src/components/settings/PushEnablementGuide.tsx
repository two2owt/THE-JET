    ...
    <div className="rounded-xl border-hairline bg-popover/40 p-3 sm:p-4 space-y-3">
      ...
      {isSubscribed ? (
        <ul className="space-y-2">
          <Step icon={Bell}>
            JET open: alerts appear as an in-app banner with a{" "}
            <span className="font-medium">View</span> button that opens the deal
            or venue card.
          </Step>
          <Step icon={Smartphone}>
            JET closed or in the background: alerts arrive on your lock screen —
            tapping one opens JET straight to that deal.
          </Step>
          <Step icon={CheckCircle2}>
            Every alert is also saved to the{" "}
            <span className="font-medium">Alerts</span> tab, so nothing is
            missed.
          </Step>
        </ul>
      ) : blocked ? (
        <ol className="space-y-2 list-decimal pl-5 text-xs sm:text-sm text-foreground/80">
          ...
          <li>
            Switch to <span className="font-medium">Allow</span>, reload JET,
            then flip the toggle above.
          </li>
        </ol>
      ) : needsInstallFirst ? (
        <ul className="space-y-2">
          ...
          <Step icon={Bell}>
            3. Reopen JET from your Home Screen and tap Enable — iPhone and iPad
            only deliver alerts to installed apps.
          </Step>
        </ul>
      ) : (
        <ul className="space-y-2">
          <Step icon={Bell}>
            Tap Enable, then choose <span className="font-medium">Allow</span>{" "}
            in your phone's popup.
          </Step>
          <Step icon={Smartphone}>
            Android and desktop deliver alerts even when JET is closed.
          </Step>
          <Step icon={CheckCircle2}>
            While JET is open, alerts show in-app instead of on your lock
            screen.
          </Step>
        </ul>
      )}
