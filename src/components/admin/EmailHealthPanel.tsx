...
type StatusFilter = "all" | "problems" | "sent" | "failed" | "dlq" | "suppressed" | "bounced" | "pending";

const RANGE_HOURS: Record<RangeKey, number> = { "24h": 24, "7d": 24 * 7, "30d": 24 * 30 };
const PROBLEM_STATUSES = ["failed", "dlq", "bounced", "complained", "suppressed"];

function statusVariant(status: string): "default" | "secondary" | "destructive" | "outline" {
  if (status === "sent") return "default";
  if (status === "pending") return "secondary";
  if (status === "failed" || status === "dlq" || status === "bounced" || status === "complained") return "destructive";
  return "outline";
}
...
function dedupe(rows: LogRow[]): LogRow[] {
  ...
  return Array.from(latest.entries())
    .map(([key, row]) => ({ ...row, __attempts: attempts.get(key) ?? 1 } as LogRow & { __attempts: number }))
...
export function EmailHealthPanel() {
  ...
  const stats = useMemo(() => {
    const s = { total: 0, sent: 0, pending: 0, failed: 0, suppressed: 0, retried: 0 };
  ...
  const filtered = useMemo(() => {
    return deduped.filter((r) => {
      ...
      if (status === "failed") return r.status === "failed" || r.status === "dlq";
          ...
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-destructive">
                {stats.failed} failed {stats.failed === 1 ? "send" : "sends"} in the last {range}
                {failureRate > 0 ? ` (${failureRate}% of all sends)` : ""}
              </p>
              <p className="text-xs text-muted-foreground">
                Failed and dead-lettered emails never reached the recipient. Check the error column below.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            <span
              ...
              title={live ? "Streaming new sends live" : "Reconnecting — polling every 30s"}
            >
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select value={status} onValueChange={(v) => setStatus(v as StatusFilter)}>
              <SelectTrigger className="h-9 w-[140px]" aria-label="Status filter">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <Select value={template} onValueChange={setTemplate}>
              <SelectTrigger className="h-9 w-[150px]" aria-label="Template filter">
            ...
            <>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
                {[
                  ...
                  { label: "Sent", value: stats.sent, tone: "text-emerald-400" },
                  { label: "Failed / DLQ", value: stats.failed, tone: "text-destructive" },
                  { label: "Retried", value: stats.retried, tone: "text-amber-400" },
                  { label: "Suppressed", value: stats.suppressed, tone: "text-muted-foreground" },
                ].map((s) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            {status === "problems" ? "Problem sends" : "Recent sends"} ({filtered.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">
              No {status === "problems" ? "failed, retried, or suppressed" : "matching"} sends in this window.
            </p>
          ) : (
              ...
              <table className="w-full min-w-[640px] text-sm">
                ...
                <tbody>
                  {filtered.slice(0, 100).map((r) => {
                    const attempts = (r as LogRow & { __attempts?: number }).__attempts ?? 1;
                    return (
                      <tr key={r.id} className="border-t border-border/50 align-top">
                        <td className="py-2 pr-3">
                          <Badge variant={statusVariant(r.status)}>{r.status}</Badge>
                          {attempts > 2 && (
                            <Badge variant="outline" className="ml-1 text-amber-400">
                              {attempts - 1} retries
                            </Badge>
                          )}
                        </td>
                        <td className="py-2 pr-3 font-medium">{r.template_name}</td>
                        <td className="py-2 pr-3 text-muted-foreground break-all">{r.recipient_email}</td>
