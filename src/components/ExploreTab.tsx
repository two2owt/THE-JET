...
export const ExploreTab = ({ onVenueSelect }: ExploreTabProps) => {
  ...
  return (
    ...
    <div className="flex flex-col gap-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-extrabold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent mb-2 leading-tight">
          Explore Deals
        </h2>
      ...
      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-muted-foreground" style={{ width: '18px', height: '18px' }} />
        <Input
          type="text"
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { icon: TrendingUp, value: filteredDeals.length, label: userLocation ? "Nearby Deals" : "Active Deals", iconClass: "text-primary", bgClass: "bg-primary/10 border-primary/25" },
          { icon: MapPin, value: new Set(filteredDeals.map(d => d.venue_name)).size, label: userLocation ? "Nearby Venues" : "Venues", iconClass: "text-accent", bgClass: "bg-accent/10 border-accent/25" },
          { icon: Clock, value: filteredDeals.length, label: searchQuery || selectedCategories.length > 0 ? "Results" : "Available", iconClass: "text-muted-foreground", bgClass: "bg-muted/50 border-border/50" },
        ].map((stat, i) => (
          <div
            key={i}
            className="py-4 px-2 text-center rounded-[14px] bg-card/90 border border-border/50 backdrop-blur-[8px]"
          >
            <div className={`w-10 h-10 mx-auto mb-2 rounded-full flex items-center justify-center border ${stat.bgClass}`}>
