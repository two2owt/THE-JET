...
export const useDeals = (
  enablePreferenceFilter: boolean = false,
  ...
  const filterDealsByPreferences = useCallback(
    (allDeals: Deal[], prefs: UserPreferences | null) => {
      ...
      return allDeals.filter((deal) => {
        ...
        if ((dealCategory === "Food" || dealType === "food") && prefs.food) {
          ...
          if (cuisineType.length > 0 || mealOccasion.length > 0) {
            const matchesCuisine =
              ...
              cuisineType.some((c) => searchText.includes(c.toLowerCase()));
            const matchesMeal =
              mealOccasion.length === 0 ||
              mealOccasion.some((m) => searchText.includes(m.toLowerCase()));
        ...
        if (
          (dealCategory === "Drinks" || dealType === "drinks") &&
          prefs.drink
        ) {
          ...
          if (barCocktail.length > 0 || coffeeTea.length > 0) {
            const matchesBar =
              barCocktail.length === 0 ||
              barCocktail.some((b) => searchText.includes(b.toLowerCase()));
            const matchesCoffee =
              coffeeTea.length === 0 ||
              coffeeTea.some((c) => searchText.includes(c.toLowerCase()));
        ...
        if (
          (dealCategory === "Nightlife" || dealType === "nightlife") &&
          prefs.nightlife
        ) {
          ...
          if (venueType.length > 0 || musicPreference.length > 0) {
            const matchesVenue =
              venueType.length === 0 ||
              venueType.some((v) => searchText.includes(v.toLowerCase()));
            const matchesMusic =
              musicPreference.length === 0 ||
              musicPreference.some((m) => searchText.includes(m.toLowerCase()));
        ...
        if (
          (dealCategory === "Events" || dealType === "events") &&
          prefs.events
        ) {
  ...
  const loadDeals = useCallback(async () => {
    try {
      ...
      if (
        prefetchedDeals &&
        Array.isArray(prefetchedDeals) &&
        prefetchedDeals.length > 0
      ) {
        // Use prefetched data for instant initial render
        const now = new Date().toISOString();
        const validDeals = prefetchedDeals.filter(
          (deal: Deal) =>
            deal.active && deal.expires_at >= now && deal.starts_at <= now,
        );
