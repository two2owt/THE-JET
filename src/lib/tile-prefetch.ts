 ...
 */
export function storeLastKnownLocation(
  lat: number,
  lng: number,
  city?: string,
): void {
...
async function prefetchTilesForLocation(
  ...
  if (import.meta.env.DEV)
    console.log(
      `Tile prefetch: Starting for ${locationName || `${lat.toFixed(4)}, ${lng.toFixed(4)}`}`,
    );
...
export async function prefetchTilesForLastLocation(): Promise<void> {
  ...
  if (import.meta.env.DEV)
    console.log(
      `Tile prefetch: Completed ${result.success}/${result.total} tiles`,
    );
...
export function requestServiceWorkerPrefetch(): void {
  ...
  for (const zoom of PREFETCH_ZOOM_LEVELS.slice(0, 3)) {
    // Only first 3 zoom levels for SW
