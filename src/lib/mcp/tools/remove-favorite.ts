...
export default defineTool({
  ...
  inputSchema: {
    favorite_id: z.string().uuid().optional().describe("Id of the favorite row to remove."),
    venue_id: z.string().trim().optional().describe("Venue id to remove from favorites when the favorite id is unknown."),
  },
  ...
  handler: async ({ favorite_id, venue_id }, ctx) => {
    ...
    query = favorite_id ? query.eq("id", favorite_id) : query.eq("venue_id", venue_id as string);
